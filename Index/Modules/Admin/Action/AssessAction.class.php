...
class AssessAction extends CommonAction{
    ...
    public function index(){
        ...
        $count = M('assess')->where($where)->count(); //计算总共条数

        $page = new Page($count,$limit);
        $limit = $page->firstRow . ',' . $page->listRows;
        $this->page = $page->show();
        
        $this->assess = M('assess')->limit($limit)->where($where)->order('time desc')->select();
        $this->display('index');
    }
    ...
    /*添加职业测评表单*/
    public function addAssessHandle(){
        $data = array(
            'title' => $_POST['title'],
       ...
       if($bid = M('assess')->add($data)){
            $this->success("添加成功",U(GROUP_NAME.'/Assess/index'));
       }else{
            $this->error('添加失败');
       }
    }
    ...
    public function toTrach($type){
        ...
        if(M('assess')->save($update)){
            $this->success($msg.'成功',U(GROUP_NAME . '/Assess/index'));
        }else{
            $this->error($msg.'失败');
        }
    }
    ...
    public function delete(){
        $id = (int)$_GET['id'];
        if(M('assess')->delete($id)){
            $this->success('删除成功',U(GROUP_NAME . '/Assess/trash'));
        }else{
            $this->error('删除失败');
        }
    }
