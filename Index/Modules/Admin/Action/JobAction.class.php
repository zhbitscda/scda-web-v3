...
class JobAction extends CommonAction{
    ...
    public function index(){
        ...
        $count = M('job')->where($where)->count(); //计算总共条数

        $page = new Page($count,$limit);
        $limit = $page->firstRow . ',' . $page->listRows;
        $this->page = $page->show();
        
        $this->job = M('job')->limit($limit)->where($where)->order('time desc')->select();
        $this->display('index');
    }
    ...
    public function toTrach($type){
        ...
        if(M('job')->save($update)){
            $this->success($msg.'成功',U(GROUP_NAME . '/Job/index'));
        }else{
            $this->error($msg.'失败');
        }
    }
    ...
    public function delete(){
        $id = (int)$_GET['id'];
        if(M('Job')->delete($id)){
            $this->success('删除成功',U(GROUP_NAME . '/Job/trash'));
        }else{
            $this->error('删除失败');
        }
    }
