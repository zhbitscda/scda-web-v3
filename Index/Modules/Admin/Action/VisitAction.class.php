...
class VisitAction extends CommonAction{
    ...
    public function index(){
        ...
        $count = M('visit')->where($where)->count(); //计算总共条数

        $page = new Page($count,$limit);
        $limit = $page->firstRow . ',' . $page->listRows;
        $this->page = $page->show();
        
        $this->visit = M('visit')->limit($limit)->where($where)->order('time desc')->select();
        $this->display('index');
    }
    ...
    /*添加企业观光表单*/
    public function addVisitHandle(){
        $data = array(
            'title' => $_POST['title'],
       ...
       if($bid = M('visit')->add($data)){
            $this->success("添加成功",U(GROUP_NAME.'/Visit/index'));
       }else{
            $this->error('添加失败');
       }
    }
    ...
    public function toTrach($type){
        ...
        if(M('visit')->save($update)){
            $this->success($msg.'成功',U(GROUP_NAME . '/Visit/index'));
        }else{
            $this->error($msg.'失败');
        }
    }
    ...
    public function delete(){
        $id = (int)$_GET['id'];
        if(M('visit')->delete($id)){
            $this->success('删除成功',U(GROUP_NAME . '/Visit/trash'));
        }else{
            $this->error('删除失败');
        }
    }
